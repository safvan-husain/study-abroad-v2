...
export function workspaceTargetIsAvailable(target: UiTargetRef, workSetIds: Iterable<string>): boolean {
  if (!target.workSetId) return true;
  return new Set(workSetIds).has(target.workSetId);
}
...
export function workspaceContainsTarget(current: UiTargetRef, candidate: UiTargetRef): boolean {
  ...
  if (current.workSetId && current.workSetId === candidate.workSetId) return true;
  return false;
}

export function targetBreadcrumb(target: UiTargetRef): string {
  if (target.viewType === 'home') return 'EXPLORE';
  ...
  if (target.viewType === 'documents') return 'DOCUMENTS';
