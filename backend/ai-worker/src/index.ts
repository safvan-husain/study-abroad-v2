...
import type { WorkerMessageStore } from './services/mongo-message-store.js';
...
export function createWorker(deps: { store: WorkerMessageStore; coordinator: Coordinator; jobs?: PendingJobSource }, config = loadConfig()) {
...
// The host supplies the generated SpacetimeDB connection and Mongo adapter.
// Keeping this factory injectable makes local tests independent of live services.
export async function startWorker(deps: { store: WorkerMessageStore; coordinator: Coordinator; jobs?: PendingJobSource }) {
