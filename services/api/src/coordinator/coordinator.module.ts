...
const fake: CoordinatorPort = { async enqueue(i) { const result = { ...i, status: 'pending' as const }; statuses.set(`${i.conversationId}:${i.turnId}`, Promise.resolve(result)); return result; }, async status(c, t) { const result = await statuses.get(`${c}:${t}`); if (!result) throw new Error('not found'); return result; } };
