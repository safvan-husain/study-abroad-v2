...
export interface CoordinatorPort { enqueue(input: { conversationId: string; turnId: string; correlationId: string }): Promise<TurnStatus>; status(conversationId: string, turnId: string): Promise<TurnStatus>; }

@Injectable()
export class CoordinatorService {
  constructor(private readonly coordinator: CoordinatorPort) {}
  async createTurn(input: { conversationId: string; turnId: string; correlationId: string }) {
